...
public class BookRestService {
    ...
    public <T> T postBook(String url, Object request, Class<T> responseType, Map<String, ?> urlVariables) {
        return restTemplate.postForObject(host + port + url, request, responseType, urlVariables);
    }
    ...
    public <T> T updateBook(String url, HttpMethod method, long bookId, HttpEntity<?> requestEntity,
            Class<T> responseType, Map<String, ?> urlVariables) {
        return restTemplate.exchange(host + port + url + bookId, method, requestEntity, responseType, urlVariables)
                .getBody();
    }
}
